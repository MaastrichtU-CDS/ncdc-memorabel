...
def docker_wrapper(module: str):
    ...
    input_data["master"] = False
    input_data["method"] = "test_connection"
    ...
    info("Dispatching ...")
    output = dispact_rpc(db_client, input_data, module, None)
...
def run_command(command, success_message, error_message):
    ...
    info(process.stdout.decode("utf-8"))
    info(process.stderr.decode("utf-8"))
