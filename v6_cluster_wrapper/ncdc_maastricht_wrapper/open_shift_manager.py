...
def create_tasks(task_folder, input_file, output_file, task_id, algorithm_image):
    ...
    return [
        {
            ...
            "task_command": [
                "sh",
                "-c",
                f"until [[ -d {task_folder} ]]; do echo waiting for the input; sleep 10; done;"
            ],
            "commands": [
                {
...
def run_task(task_id, task_definition):
    ...
    with open(task_definition['file']) as json_file:
        ...
        if "algorithm_image" in task_definition:
    ...
    while n_tries < 20 and not check_task_status(container_info, 'Succeeded'):
