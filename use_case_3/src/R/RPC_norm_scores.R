...
RPC_norm_scores <- function(df, config, model = "memory", exclude=c()) {
  ...
  result = tryCatch({
    ...
    df <- df %>% #I'm assuming this is the name of the dataframe from where we are retrieving the data, otherwise we need to change this
      dplyr::select(id,
                    age,
                    sex,
                    birth_year,
                    ...
                    priority_memory_im_ravlt,
                    # priority_memory_dr_lm,
                    priority_memory_dr_ravlt,
                    attention_test_tmt_a_time,
      ...
      dplyr::filter(!is.na(education_category_3)
                    & !is.na(sex)
                    & !is.na(birth_year)
                    & !is.na(priority_memory_im_ravlt)
                    & !is.na(priority_memory_dr_ravlt)
                    & !is.na(attention_test_tmt_a_time)
                    & !is.na(priority_executive_tmt_b_time)) %>%  # Remove rows with missing values for any of the columns
      # dplyr::filter(scd_diagnosis | normal_cognition_diagnosis) %>% #excludes everyone who isn't either healthy or has scd
      dplyr::filter(!dementia_diagnosis) %>%
      group_by(id) %>%
      dplyr::slice_min(order_by = date) #selects the earliest date available per participant

    vtg::log$info("Data selection: '{nrow(df)}' participants")
