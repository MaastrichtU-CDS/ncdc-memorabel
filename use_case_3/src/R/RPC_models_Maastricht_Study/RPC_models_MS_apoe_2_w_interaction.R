RPC_models_apoe_2_w_int <- function(df, config, model = "memory", exclude=c()) {
  ...
  result = tryCatch({
    ...
    df_grouped <- df_grouped[! duplicated(df_grouped$id),]
    # "priority_attention_test_stroop_average_time", "priority_executive_test_tmt_b_time",
    # df <- merge(
...
# RIRS model with unstructured covariance structure (add model for every biomarker x cognitive measure)
    ...
    summary_memory_p_tau_im <- safe_lme_summary(priority_memory_im_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + p_tau + p_tau * years_since_baseline
                                      + apoe_carrier * p_tau,
                                      data = df,
    ...
    summary_language_amyloid_b_ratio <- safe_lme_summary(priority_language_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + amyloid_b_ratio_42_40
                                               + apoe_carrier * amyloid_b_ratio_42_40 + amyloid_b_ratio_42_40 * years_since_baseline,
                                               data = df,
                                               random = ~ years_since_baseline | id,
                                               weights = nlme::varIdent(form= ~1 | years_since_baseline),
    ...
    summary_processing_speed_p_tau <- safe_lme_summary(priority_processing_speed_ldst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + p_tau
    ...
    summary_processing_speed_gfap <- safe_lme_summary(priority_processing_speed_ldst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + gfap
     ...
     summary_processing_speed_nfl <- safe_lme_summary(priority_processing_speed_ldst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + nfl
    ...
    summary_processing_speed_amyloid_b_ratio <- safe_lme_summary(priority_processing_speed_ldst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + amyloid_b_ratio_42_40
    ...
    summary_attention_cst_average_p_tau <- safe_lme_summary(priority_attention_cst_average_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + p_tau
                                            + p_tau * years_since_baseline
                                            + apoe_carrier * p_tau,
                            data = df,
                            random = ~ years_since_baseline | id,
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_attention_cst_average_p_tau <- sjPlot::tab_model(summary_attention_cst_average_p_tau, digits = 10)

    vtg::log$info("summary_attention_cst_average_gfap")
    summary_attention_cst_average_gfap <- safe_lme_summary(priority_attention_cst_average_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + gfap
                                     + gfap * years_since_baseline
                                     + apoe_carrier * gfap,
                            data = df,
                            random = ~ years_since_baseline | id,
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_attention_cst_average_gfap <- sjPlot::tab_model(summary_attention_cst_average_gfap, digits = 10)

    vtg::log$info("summary_attention_cst_average_nfl")
    summary_attention_cst_average_nfl <- safe_lme_summary(priority_attention_cst_average_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + nfl
                                    + nfl * years_since_baseline
                                    + apoe_carrier * nfl,
                            data = df,
                            random = ~ years_since_baseline | id,
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_attention_cst_average_nfl <- sjPlot::tab_model(summary_attention_cst_average_nfl, digits = 10)

    vtg::log$info("summary_attention_cst_average_amyloid_b_ratio")
    summary_attention_cst_average_amyloid_b_ratio <- safe_lme_summary(priority_attention_cst_average_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + amyloid_b_ratio_42_40
                                                + amyloid_b_ratio_42_40 * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40,
                            data = df,
                            random = ~ years_since_baseline | id,
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_attention_cst_average_amyloid_b_ratio <- sjPlot::tab_model(summary_attention_cst_average_amyloid_b_ratio, digits = 10)

    #Attention (Stroop)
    vtg::log$info("summary_attention_stroop_average_p_tau")
    summary_attention_stroop_average_p_tau <- safe_lme_summary(priority_attention_stroop_average_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + p_tau
                                            + p_tau * years_since_baseline
                                            + apoe_carrier * p_tau,
                            data = df,
                            random = ~ years_since_baseline | id,
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_attention_stroop_average_p_tau <- sjPlot::tab_model(summary_attention_stroop_average_p_tau, digits = 10)

    vtg::log$info("summary_attention_stroop_average_gfap")
    summary_attention_stroop_average_gfap <- safe_lme_summary(priority_attention_stroop_average_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + gfap
                                     + gfap * years_since_baseline
                                     + apoe_carrier * gfap,
                            data = df,
                            random = ~ years_since_baseline | id,
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_attention_stroop_average_gfap <- sjPlot::tab_model(summary_attention_stroop_average_gfap, digits = 10)

    vtg::log$info("summary_attention_stroop_average_nfl")
    summary_attention_stroop_average_nfl <- safe_lme_summary(priority_attention_stroop_average_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + nfl
                                    + nfl * years_since_baseline
                                    + apoe_carrier * nfl,
                            data = df,
                            random = ~ years_since_baseline | id,
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_attention_stroop_average_nfl <- sjPlot::tab_model(summary_attention_stroop_average_nfl, digits = 10)

    vtg::log$info("summary_attention_stroop_average_amyloid_b_ratio")
    summary_attention_stroop_average_amyloid_b_ratio <- safe_lme_summary(priority_attention_stroop_average_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + amyloid_b_ratio_42_40
                                                + amyloid_b_ratio_42_40 * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40,
                            data = df,
                            random = ~ years_since_baseline | id,
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_attention_stroop_average_amyloid_b_ratio <- sjPlot::tab_model(summary_attention_stroop_average_amyloid_b_ratio, digits = 10)
    ...
    summary_executive_cst_amyloid_b_ratio <- safe_lme_summary(priority_executive_cst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + amyloid_b_ratio_42_40
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_priority_executive_cst_amyloid_b_ratio <- sjPlot::tab_model(summary_priority_executive_cst_amyloid_b_ratio, digits = 10)
    ...
    summary_executive_shifting_amyloid_b_ratio <- safe_lme_summary(priority_executive_shifting_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + amyloid_b_ratio_42_40
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_priority_executive_shifting_amyloid_b_ratio <- sjPlot::tab_model(summary_priority_executive_shifting_amyloid_b_ratio, digits = 10)
    ...
    summary_executive_stroop_3_amyloid_b_ratio <- safe_lme_summary(priority_executive_stroop_3_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + amyloid_b_ratio_42_40
                            ...
                            na.action = na.exclude,
                           control = nlme::lmeControl(opt='optim', maxIter = 500, msMaxIter = 500, msMaxEval = 500, msVerbose = TRUE))
    # summary_priority_executive_stroop_3_amyloid_b_ratio <- sjPlot::tab_model(summary_priority_executive_stroop_3_amyloid_b_ratio, digits = 10)
 ...
 results <- list(
      ...
      "summary_executive_shifting_amyloid_b_ratio" = summary_executive_shifting_amyloid_b_ratio,
