...
RPC_models_sex_2_w_interaction <- function(df, config, model = "memory", exclude=c()) {
  ...
  result = tryCatch({
    ...
    print(df$years_since_baseline)
    print(df$priority_language_animal_fluency_60_correct)
    ...
    descriptives_by_sex_table <- df %>%
      ...
      dplyr::summarise(
        ...
        mean_age = mean(age_rec, na.rm = TRUE),
        sd_age = sd(age_rec, na.rm = TRUE),
        mean_years_baseline = mean(years_since_baseline, na.rm = TRUE),
        sd_years_baseline = sd(years_since_baseline, na.rm = TRUE),
      )
