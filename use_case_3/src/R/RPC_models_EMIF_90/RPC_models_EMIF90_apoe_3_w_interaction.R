...
#This makes a table with means and standard deviations for the following variables per days since baseline
   ...
   descriptives_by_sex_and_FU_NPA_table <- df %>%
    ...
    RIRS_processing_speed_p_tau <- nlme::lme(priority_processing_speed_sdst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + p_tau 
                                             + p_tau * years_since_baseline
                                             + apoe_carrier * p_tau 
                                             + apoe_carrier * years_since_baseline
                                             + apoe_carrier * p_tau * years_since_baseline,
    ...
    RIRS_processing_speed_gfap <- nlme::lme(priority_processing_speed_sdst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + gfap 
                                     + gfap * years_since_baseline
                                     + apoe_carrier * gfap 
                                     + apoe_carrier * years_since_baseline
                                     + apoe_carrier * gfap * years_since_baseline,
    ...
    RIRS_processing_speed_nfl <- nlme::lme(priority_processing_speed_sdst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + nfl 
                                    + nfl * years_since_baseline
                                    + apoe_carrier * nfl 
                                    + apoe_carrier * years_since_baseline
                                    + apoe_carrier * nfl * years_since_baseline,
    ...
    RIRS_processing_speed_amyloid_b_ratio <- nlme::lme(priority_processing_speed_sdst_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + amyloid_b_ratio_42_40 
                                                + amyloid_b_ratio_42_40 * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40 
                                                + apoe_carrier * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40 * years_since_baseline,
    ...
    RIRS_attention_tmt_a_p_tau <- nlme::lme(priority_attention_tmt_a_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + p_tau 
                                            + p_tau * years_since_baseline
                                            + apoe_carrier * p_tau 
                                            + apoe_carrier * years_since_baseline
                                            + apoe_carrier * p_tau * years_since_baseline,
    ...
    RIRS_attention_tmt_a_gfap <- nlme::lme(priority_attention_tmt_a_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + gfap 
                                     + gfap * years_since_baseline
                                     + apoe_carrier * gfap 
                                     + apoe_carrier * years_since_baseline
                                     + apoe_carrier * gfap * years_since_baseline,
    ...
    RIRS_attention_tmt_a_nfl <- nlme::lme(priority_attention_tmt_a_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + nfl 
                                    + nfl * years_since_baseline
                                    + apoe_carrier * nfl 
                                    + apoe_carrier * years_since_baseline
                                    + apoe_carrier * nfl * years_since_baseline,
    ...
    RIRS_attention_tmt_a_amyloid_b_ratio <- nlme::lme(priority_attention_tmt_a_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + amyloid_b_ratio_42_40 
                                                + amyloid_b_ratio_42_40 * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40 
                                                + apoe_carrier * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40 * years_since_baseline,
    ...
    RIRS_priority_executive_tmt_b_p_tau <- nlme::lme(priority_executive_tmt_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + p_tau 
                                        + p_tau * years_since_baseline
                                        + apoe_carrier * p_tau 
                                        + apoe_carrier * years_since_baseline
                                        + apoe_carrier * p_tau * years_since_baseline,
    ...
    RIRS_priority_executive_tmt_b_gfap <- nlme::lme(priority_executive_tmt_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + gfap 
                                     + gfap * years_since_baseline
                                     + apoe_carrier * gfap 
                                     + apoe_carrier * years_since_baseline
                                     + apoe_carrier * gfap * years_since_baseline,
    ...
    RIRS_priority_executive_tmt_b_nfl <- nlme::lme(priority_executive_tmt_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + nfl 
                                    + nfl * years_since_baseline
                                    + apoe_carrier * nfl 
                                    + apoe_carrier * years_since_baseline
                                    + apoe_carrier * nfl * years_since_baseline,
    ...
    RIRS_priority_executive_tmt_b_amyloid_b_ratio <- nlme::lme(priority_executive_tmt_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + amyloid_b_ratio_42_40 
                                                + amyloid_b_ratio_42_40 * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40 
                                                + apoe_carrier * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40 * years_since_baseline,
    ...
    RIRS_priority_executive_shift_tmt_z_p_tau <- nlme::lme(priority_executive_tmt_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + p_tau 
                                      + p_tau * years_since_baseline
                                      + apoe_carrier * p_tau 
                                      + apoe_carrier * years_since_baseline
                                      + apoe_carrier * p_tau * years_since_baseline,
    ...
    RIRS_priority_executive_shift_tmt_z_gfap <- nlme::lme(priority_executive_tmt_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + gfap 
                                     + gfap * years_since_baseline
                                     + apoe_carrier * gfap 
                                     + apoe_carrier * years_since_baseline
                                     + apoe_carrier * gfap * years_since_baseline,
    ...
    RIRS_priority_executive_shift_tmt_z_nfl <- nlme::lme(priority_executive_tmt_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + nfl 
                                    + nfl * years_since_baseline
                                    + apoe_carrier * nfl 
                                    + apoe_carrier * years_since_baseline
                                    + apoe_carrier * nfl * years_since_baseline,
    ...
    RIRS_priority_executive_shift_tmt_z_amyloid_b_ratio <- nlme::lme(priority_executive_tmt_z ~ years_since_baseline + age_rec + sex + sqrt_prior_visit + education_low + education_high + apoe_carrier + apoe_carrier + amyloid_b_ratio_42_40 
                                                + amyloid_b_ratio_42_40 * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40 
                                                + apoe_carrier * years_since_baseline
                                                + apoe_carrier * amyloid_b_ratio_42_40 * years_since_baseline,
